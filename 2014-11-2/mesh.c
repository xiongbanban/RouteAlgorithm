...
static void ieee80211_mesh_housekeeping_timer(unsigned long data)
{
	...
	ieee80211_queue_work(&local->hw, &sdata->work); //60s
}
...
void mesh_sta_cleanup(struct sta_info *sta)
{
	...
	if (!sdata->u.mesh.user_mpm) {
		...
		// É¾µôplinkµÄ¶¨Ê±Æ÷ºó ¾ÍÏàµ±ÓÚÉ¾µôÁËplinkµÄÐÅÏ¢
	}
 ...
 * it.
 */
int timermesh_rmc_check(struct ieee80211_sub_if_data *sdata,
...
static void ieee80211_mesh_path_timer(unsigned long data)
	//ÓÃÀ´½«µÈ´ýÒ»¶ÎÊ±¼äºóµÄmesh-preq-queue Ìí¼Ó½ø¹¤×÷¶ÓÁÐ
{
...

{
	...
	set_bit(MESH_WORK_ROOT, &ifmsh->wrkq_flags);//ÉèÖÃrootÎ»
...
static void ieee80211_mesh_housekeeping(struct ieee80211_sub_if_data *sdata)
{
    ...
    // Ò»·ÖÖÓÒ»´Î É¾µô¹ýÆÚµÄÁÙ½ü½ÚµãstaµÄÐÅÏ¢
	mesh_path_expire(sdata);
    // 10·ÖÖÓÒ»´Î É¾µô¹ýÆÚµÄÂ·¾¶ÐÅÏ¢
	changed = mesh_accept_plinks_update(sdata);
	// ¸üÐÂplinksµÄÐÅÏ¢
	ieee80211_mbss_info_change_notify(sdata, changed);
	...
	mod_timer(&ifmsh->housekeeping_timer,
		  round_jiffies(jiffies +
				IEEE80211_MESH_HOUSEKEEPING_INTERVAL));//60s
}
...
int ieee80211_start_mesh(struct ieee80211_sub_if_data *sdata)
{
	...
	ieee80211_queue_work(&local->hw, &sdata->work);//¿ªÊ¼¹¤×÷ ·¢ËÍpreq
	sdata->vif.bss_conf.ht_operation_mode =
...
void ieee80211_mesh_work(struct ieee80211_sub_if_data *sdata)
{
	...
	if (ifmsh->preq_queue_len &&
		...
		mesh_path_start_discovery(sdata);//¿ªÊ¼·¢ÏÖ
	...
	if (test_and_clear_bit(MESH_WORK_HOUSEKEEPING, &ifmsh->wrkq_flags))
		ieee80211_mesh_housekeeping(sdata);//Æô¶¯housekeepingµÄtimer

	if (test_and_clear_bit(MESH_WORK_ROOT, &ifmsh->wrkq_flags))
		ieee80211_mesh_rootpath(sdata);//ÔÚmesh workÀï
...
void ieee80211_mesh_init_sdata(struct ieee80211_sub_if_data *sdata)
{
	...
	(&ifmsh->ps.bc_buf);
	spin_loskb_queue_head_initck_init(&ifmsh->mesh_preq_queue_lock);
