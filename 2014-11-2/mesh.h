...
struct mesh_path {
	...
	struct sta_info __rcu *next_hop;//This is an annotation for a the Sparse code analysis tool that can warn about certain things the programmer may have overlooked.
...
struct mesh_preq_state_macchine_table  *preq_table[NODE_NUM];//目的节点接受preq的头结点
...
u8 mesh_preq_mid_list_hash(u8 *node1_addr, u8 *node2_addr);// ��ȡ���һ���ֽ� ������ ��hash
...
void preq_state_machine_add(u8 * orig_addr); //这里就在链表上添加一个节�?

void preq_state_machine_timer(unsigned long data);   //timer  �?

void preq_state_machine_clean_timer(unsigned long data);   //timer  �?并发 很重�?
...
int mesh_path_error_tx(struct ieee80211_sub_if_data *sdata,
		       ...
		       u16 target_rcode, const u8 *ra,const u8 *orig_addr,u8 * invalid_addr,u8 flag);
