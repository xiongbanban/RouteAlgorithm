...
#define MP_F_MAJOR  0x03
#define MP_F_MINOR  0x04
...
int mesh_path_error_tx(struct ieee80211_sub_if_data *sdata,
		       ...
		       ,const u8 *orig_addr,u8 * invalid_addr,u8 flag)
{
	struct ieee80211_local *local = sdata->local;
	struct sk_buff *skb;
				    ...
				    const u8 *preq_elem, u32 metric)
{
	...
	if (reply) {
		...
		printk("destination node process");
        destination_node_preq_process(orig_addr, 
								 ie_len,mid_address_list, metric,
 ...
 void  destination_node_preq_process(u8 * orig_addr, 
	 ...
	 if (node){
	  ...
	  //&&(!(mpath->flags & MESH_PATH_RECEIVE_PREQ_TIMEOUT)))
	   {  
           ...
           mod_timer(&node->des_req_timer,jiffies+des_req_wait_timeout);
           mod_timer(&node->des_clear_timer,jiffies+des_req_wait_timeout+des_req_clear_timeout);
...
void  path_info_add (struct mesh_preq_state_machine * node,u8 ie_len,char * mid_address_list,u32 metric,u32 lifetime,
...
preq_state_machine_timer(unsigned long data)   //timer  锁 
{
    ...
    if(major_path){
		...
		mpath = mesh_major_path_lookup(major_path->sdata, node->orig_addr,major_path->sdata->vif.addr);
        if(!mpath){
          mpath = mesh_path_add(major_path->sdata, MESH_PATHS,node->orig_addr,major_path->sdata->vif.addr);
          if (IS_ERR(mpath)) {
				return ;
		   }
		}
    ...
    if(minor_path){
        ...
        mpath = mesh_minor_path_lookup(minor_path->sdata, node->orig_addr,minor_path->sdata->vif.addr);
        if(!mpath){
          mpath = mesh_path_add(minor_path->sdata, MESH_PATHS_BACKUP,node->orig_addr,minor_path->sdata->vif.addr);
          if (IS_ERR(mpath)) {
				return ;
		   }
		}
				    ...
				    const u8 *prep_elem, u32 metric) // 发送之前要删除地址列表中的最后一个地址 lengh变
{
    ...
    printk("send prep to %pM",target_addr);	
				    ...
				    const u8 *perr_elem)
{
	...
	if(target_sn ==0 && ether_addr_equal(orig_addr,sdata->vif.addr)){
	  printk("receive perr major path from %pM to %pM", target_addr,orig_addr);
		...
		if (sta && mpath->flags & MESH_PATH_ACTIVE &&
			...
			if (!ifmsh->mshcfg.dot11MeshForwarding)
			  ...
			  if (!ether_addr_equal(orig_addr,sdata->vif.addr)){
			    ...
			    orig_path=mesh_major_path_lookup(sdata,orig_address ,target_address);
		...
		if (sta && mpath->flags & MESH_PATH_ACTIVE &&
			...
			if (!ifmsh->mshcfg.dot11MeshForwarding)
			  ...
			  if (!ether_addr_equal(orig_addr,sdata->vif.addr)){
			    ...
			    orig_path=mesh_minor_path_lookup(sdata,orig_address ,target_address);
                if(orig_path && (orig_path->flags & MESH_PATH_ACTIVE) &&
				    next_hop_deref_protected(orig_path)){
