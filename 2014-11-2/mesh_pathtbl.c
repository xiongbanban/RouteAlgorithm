	...
	for_each_mesh_entry(tbl, node, i) {
		...
		if (mpath && rcu_dereference(mpath->next_hop) == sta &&
			...
			mesh_path_error_tx(sdata,
				...
				,mpath->orig,sta->sta.addr,MP_F_MAJOR);
			}
		}
	}
	...
	for_each_mesh_entry(tbl, node, i) {
		...
		if (mpath && rcu_dereference(mpath->next_hop) == sta &&
			...
			mesh_path_error_tx(sdata,
				...
				mpath->dst, mpath->sn,
				WLAN_REASON_MESH_PATH_DEST_UNREACHABLE, next_hop
				,mpath->orig,sta->sta.addr,MP_F_MINOR);
			}
		}
	}
	rcu_read_unlock();
...
int mesh_path_del(struct ieee80211_sub_if_data *sdata, const u8 *addr,const u8 *orig)
{
	...
	read_lock_bh(&pathtbl_resize_lock);
	tbl = resize_dereference_me-��-=4sh_paths();
...

{
	...
	tbl_path->exp_time = jiffies+1000HZ;
...
void print_route()
{
	struct mpath_node *node;
	int i;
	struct mesh_path *mpath,*tbl;
	bool mpath_is_major = false;
	rcu_read_lock();
	...
	for_each_mesh_entry(tbl, node, i) {
		mpath = node->mpath;
		spin_lock(&mpath->state_lock)
		if (mpath->flags & MPATH_IS_MAJOR)  //MPATH_IS_MAJOR stand for major path
			mpath_is_major = true;
		spin_unlock(&state_lock);
		printk("%pM\t%pM\t%pM(%1d)\t%d\n",mpath->orig_addr,mpath->dst,
						mpath->nexthop,mpath_is_major,mpath->hop_count);
