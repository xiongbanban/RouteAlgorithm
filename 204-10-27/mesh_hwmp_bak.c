...
static int mesh_path_sel_frame_tx(enum mpath_frame_type action, u8 flags,
		   ...
		   u8 *path_address_list,  u8 ie_len,                 
				    ...
				    const u8 *preq_elem, u32 metric)
{
	...
	if (reply) {
        ...
        destination_node_preq_process(orig_addr);
								...
								u32 orig_sn, struct ieee80211_sub_if_data *sdata)
{
	...
	if(!node){
		...
		preq_state_machine_add(orig_addr,ie_len,mid_address_list,metric,lifetime,orig_sn,sdata);//
	 ...
	 if (node){
	  ...
	  //&&(!(mpath->flags & MESH_PATH_RECEIVE_PREQ_TIMEOUT)))
	   ...
	   if(!(node->flags & MESH_RECEIVE_PREQ_TIMEOUT) && ie_len>37) //no timeout
	   	//metric比主路径好 就一定更新 再判断原来的主路径是否和这条路径链路相交 不是 原来的主换成备份 
	   	//metric只比备份好 就还要判断是否和主路径相交 如果是的 就不更新
	   	//更新有两种 1 更新主 判断 再决定主换备 2 只更新备份 
		{

			 path_info_add (orig_addr,ie_len, mid_address_list,metric,lifetime,orig_sn, sdata);
...
void generate_major_path_hash_table(struct mesh_preq_state_machine * node,char * mid_address_list_new, u8 count)
...
//kind 1 指主路径 2 指 备份路径
{
	...
	//struct mesh_preq_state_machine * state_machine=mesh_get_preq_from_list(orig_addr);
	//struct mesh_preq_mid_list_hash_node *hash_node;
	//u8 hash_num;
	...
	//spin_lock_bh(&(state_machine->lock));
    ...
    if( path_minor->node_cnt>1 )
	{
		...
		{		
            ...
            {
               //spin_unlock_bh(&(state_machine->lock));
               return false;
            }
		}		
	}
	   	   ...
	   	   list_for_each_entry(path_info_for_list,&node->list_preq,list_preq)
           {
             if(SN_GT(path_info_node->orig_sn,path_info_for_list->orig_sn) ||
             	   (!(SN_GT(path_info_for_list->orig_sn,path_info_node->orig_sn)) 
             		&&SN_GT(path_info_node->metric,path_info_for_list->metric)){
             	list_add_tail(&path_info_for_list->list_preq,&path_info_node->list_preq);// 节点前插入
       	        goto end;
       	     }
           }
...
void preq_state_machine_add(u8 * orig_addr) //这里就在链表上添加一个节点 
    ...
    if(major_path){
		...
		if (ttl != 0) {
				...
				mesh_path_sel_frame_tx(MPATH_PREP, 0, node->originater_address,
						       ...
						       ,(major_path->node_cnt)*ETH_ALEN +31, major_path->sdata);
		} else {
				ifmsh->mshstats.dropped_frames_ttl++;
		}
	}
			 ...
			 struct sk_buff *skb)
{
	...
	mpath = mesh_path_lookup(sdata, target_addr,sdata->vif.address)
	if (!mpath) {

     //ÔÚÕâÀïÌí¼ÓÈç¹ûÓÐroot °ÑÊý¾ÝÏòroot		
		mpath = mesh_path_add(sdata, MESH_PATHS,target_addr,sdata->vif.address);
		if (IS_ERR(mpath)) {
			mesh_path_discard_frame(sdata, skb);
			err = PTR_ERR(mpath);
			goto endlookup;
		}
		mpath_backup= mesh_path_add(sdata, MESH_PATHS_BACKUP,target_addr,sdata->vif.address);
		if (IS_ERR(mpath)) {
			err = PTR_ERR(mpath);
			goto endlookup;
		}
			...
			struct sk_buff *skb)
{
	...
	rcu_read_lock();
	mpath = mesh_major_path_lookup(sdata, target_addr,sdata->vif.address);

	if (!mpath || !(mpath->flags & MESH_PATH_ACTIVE))
		mpath = mesh_minor_path_lookup(sdata, target_addr,sdata->vif.address);
	if (!mpath || !(mpath->flags & MESH_PATH_ACTIVE))
		goto endlookup;
