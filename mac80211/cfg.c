...
static int ieee80211_add_mpath(struct wiphy *wiphy, struct net_device *dev,
    u8 *dst, u8 *next_hop,u8 *orig_address)
{
	struct ieee80211_sub_if_data *sdata;
	...
	mpath = mesh_path_add(sdata, MESH_PATHS,dst,orig_address);
	if (IS_ERR(mpath)) {
		rcu_read_unlock();
			       ...
			       u8 *dst)
{
	...
	if (dst)
		return mesh_path_del(sdata, dst,sdata->vif.addr);
				    ...
				    u8 *dst, u8 *next_hop)
{
	...
	mpath = mesh_path_lookup(sdata, dst,sdata->vif.addr);
	if (!mpath) {
		rcu_read_unlock();
		return -ENOENT;
	}
