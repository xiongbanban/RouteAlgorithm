...
extern  struct mesh_table __rcu *mesh_paths;
extern  struct mesh_table __rcu *mesh_paths_backup;//备份路由表
...
extern  struct mesh_preq_state_macchine_table preq_table[];//??桃??节??寞?亙??preq的憭渡??点
...
struct mesh_preq_mid_list_hash_node // 瘥?銝泅tate_machine里??Ｖ葵指针指向銝?銝沼ash  table
{
	u8 *node1_addr;
	u8 *node2_addr;
	struct list_head list;  //初憪?化
    u8  flags; // ??斗?剜?臬?血??在  撠梁??摮?在 銋?閬?敺芰?舫?曇” ??脫迫hash??脩??
	//spinlock_t  lock;     //初憪?化
};



enum mesh_perr_flgas
{
    MP_F_MAJOR=0x03,
    MP_F_MINOR=0x04,
};
...
struct mesh_preq_state_machine
{
    ...
    //指向??冽??分配的??冽?亙?斗?剝?曇楝銝???喃漱的hash銵?
    //餈?銝芸?牢hoose_major_path里??Ｗ??配内摮?初憪?化 

    //struct mesh_preq_mid_list_hash_node* mesh_preq_mid_list_hash_tabel_minor;//??男malloc的??嗅????箏??憭批??鈭?
...
u8 mesh_preq_mid_list_hash(u8 *node1_addr, u8 *node2_addr);// 跪郔綴珨跺趼誹 樓懂 酕hash


bool mesh_preq_mid_list_hash_tabel_has(struct mesh_preq_state_machine * node,u8 *node1_addr, u8 *node2_addr);//
...
struct path_info * choose_minor_path(struct mesh_preq_state_machine * state_machine);

struct path_info * choose_major_path(struct mesh_preq_state_machine * state_machine);
...
void preq_state_machine_add(const u8 * orig_addr); //餈?里撠勗?券?曇”銝?瘛餃??銝?銝芾?????

void preq_state_machine_timer(unsigned long data);   //timer  ???

void preq_state_machine_clean_timer(unsigned long data);   //timer  ???撟嗅?? 敺?重閬?
...
struct mesh_path *mesh_major_path_lookup(struct ieee80211_sub_if_data *sdata,
    const u8 *dst,const u8* orig);
struct mesh_path *mesh_minor_path_lookup(struct ieee80211_sub_if_data *sdata,
    const u8 *dst,const u8* orig);
...
struct mesh_path *
mesh_path_add(struct ieee80211_sub_if_data *sdata,enum mesh_table_choose flags, const u8 *dst,const u8 *orig);
...
int mesh_path_error_tx(struct ieee80211_sub_if_data *sdata,
		       ...
		       u16 target_rcode, const u8 *ra,const u8 *orig_addr,const u8 *invalid_addr,enum mesh_perr_flgas flag);
