...
struct mpath_node {
 ...
 struct mesh_table __rcu *mesh_paths_backup;//备份路由表

static struct mesh_table __rcu *mpp_paths; /* Store paths for MPP&MAP */
struct mesh_preq_state_macchine_table  preq_table[NODE_NUM];//??桃??节??寞?亙??preq的憭渡??点
...
static u32 mesh_table_hash3(const u8 *addr, const u8 *orig,struct ieee80211_sub_if_data *sdata,
...
static u32 mesh_table_hash2(const u8 *addr, struct ieee80211_sub_if_data *sdata,
			   struct mesh_table *tbl)
{
	/* Use last four bytes of hw addr and interface index as hash index */
				      ...
				      struct ieee80211_sub_if_data *sdata,const u8* orig)
{
	...
	bucket = &tbl->hash_buckets[mesh_table_hash3(dst,orig, sdata, tbl)];
	hlist_for_each_entry_rcu(node, bucket, list) {
		mpath = node->mpath;
		if (mpath->sdata == sdata &&
		    ether_addr_equal(dst, mpath->dst) && 
		    ether_addr_equal(orig, mpath->orig) ) {
...
mpp_path_lookup(struct ieee80211_sub_if_data *sdata, const u8 *dst)
{
	...
	bucket = &tbl->hash_buckets[mesh_table_hash2(dst,sdata, tbl)];
	hlist_for_each_entry_rcu(node, bucket, list) {
		mpath = node->mpath;
		if (mpath->sdata == sdata &&
		    ether_addr_equal(dst, mpath->dst)) {
				...
				const u8 *dst,const u8 *orig)
{
	...
	hash_idx = mesh_table_hash3(dst,orig, sdata, tbl);
		 ...
		 const u8 *dst, const u8 *mpp)
{
	...
	hash_idx = mesh_table_hash2(dst, sdata, tbl);
	bucket = &tbl->hash_buckets[hash_idx];
...
void mesh_plink_broken(struct sta_info *sta)
{
	...
	u8 orig_address[ETH_ALEN];
	u8 target_address[ETH_ALEN];
	u32 sn;

	rcu_read_lock();
	tbl = rcu_dereference(mesh_paths);
	for_each_mesh_entry(tbl, node, i) {
		// only for major path,,the same for minor path 
		mpath = node->mpath;
		if (mpath && rcu_dereference(mpath->next_hop) == sta &&
		    mpath->flags & MESH_PATH_ACTIVE &&
		    !(mpath->flags & MESH_PATH_FIXED)) {
			spin_lock_bh(&mpath->state_lock);
			mpath->flags &= ~MESH_PATH_ACTIVE;
			++mpath->sn;
			memcpy(orig_address,mpath->orig,ETH_ALEN);
			memcpy(target_address,mpath->dst,ETH_ALEN);
			sn=mpath->sn;
			spin_unlock_bh(&mpath->state_lock);

		    orig_path=mpath_lookup(tbl,orig_address, sdata,target_address);//
			...
			//xf
            printk("plink error major send frome %pM to %pM\n",mpath->orig,mpath->dst);
			mesh_path_error_tx(sdata,
				sdata->u.mesh.mshcfg.element_ttl,
				target_address,sn,
				WLAN_REASON_MESH_PATH_DEST_UNREACHABLE, next_hop
				,orig_address,sta->sta.addr,MP_F_MAJOR);
			}
		}
	}


	tbl = rcu_dereference(mesh_paths_backup);
	for_each_mesh_entry(tbl, node, i) {
		// only for minor path 
		mpath = node->mpath;
		if (mpath && rcu_dereference(mpath->next_hop) == sta &&
            ...
            //xf
			printk("plink error minor send frome %pM to %pM\n",mpath->orig,mpath->dst);

			mesh_path_error_tx(sdata,
				sdata->u.mesh.mshcfg.element_ttl,
				target_address, sn,
				WLAN_REASON_MESH_PATH_DEST_UNREACHABLE, next_hop
				,orig_address,sta->sta.addr,MP_F_MINOR);
			}
		}
	}
	rcu_read_unlock();
...
static void __mesh_path_del(struct mesh_table *tbl, struct mpath_node *node)
{
	...
	call_rcu(&node->rcu, mesh_path_node_reclaim);// kfree
 ...
 */
int mesh_path_del(struct ieee80211_sub_if_data *sdata, const u8 *addr,const u8 *orig)
{
	struct mesh_table *tbl;
	struct mesh_path *mpath;
	struct mpath_node *node;
	...
	read_lock_bh(&pathtbl_resize_lock);
	tbl = resize_dereference_mesh_paths();
	hash_idx = mesh_table_hash3(addr,orig, sdata, tbl);
	bucket = &tbl->hash_buckets[hash_idx];
...
static void mesh_path_node_free(struct hlist_node *p, bool free_leafs)
{
	...
	hlist_del_rcu(p);
	if (free_leafs) {
		del_timer_sync(&mpath->timer);// ?ǵ?ɾ????ʱ?? sync
		kfree(mpath);
	}
	kfree(node);
}
...
static int mesh_path_node_copy(struct hlist_node *p, struct mesh_table *newtbl)
{
	...
	hash_idx = mesh_table_hash3(mpath->dst,mpath->orig,mpath->sdata, newtbl);
...
{
	struct mesh_table *tbl_path, *tbl_mpp, *tbl_path_backup;
	...
	spin_lock_init(&(preq_table[i].state_lock));
    INIT_LIST_HEAD(&(preq_table[i].list));
    }
...
//	tbl_path->timer.data=(unsigned long )tbl_path;
//   tbl_path->timer.function = mesh_pathtb_show_timer;
//	tbl_path->exp_time = jiffies+1000;
//	init_timer(&tbl_path->timer);
...
free_path_backup:
    mesh_table_free(tbl_path_backup,true);
...
//void mesh_pathtb_show_timer(unsigned long data)
//{
    /*struct mesh_table *table=(void *) data;
	struct mesh_path *mpath;
	struct hlist_head *bucket;
	struct mpath_node *node;
	u8 i,j;

	printk("mpath table show timer \n");

    for (i = 0; i <= newtbl->hash_mask; i++){
	bucket = &table->hash_buckets[i];
	hlist_for_each_entry_rcu(node, bucket, list) {
		    mpath = node->mpath;
			...
			for(j=0;j<ETH_ALEN;j++)
			   printk("%u",mpath->orig[j]);
			printk("\ndes");
			for(j=0;j<ETH_ALEN;j++)
			   printk("%u",mpath->dst[j]);
			printk("\nflags=%u\n",mpath->flags &= ~MESH_PATH_ACTIVE
			spin_unlock_bh(&mpath->state_lock);
    ...
    mod_timer(&table->timer,jiffies+1000HZ);*/
//}
...
void mesh_path_expire(struct ieee80211_sub_if_data *sdata)
{
        ...
        for_each_mesh_entry(tbl, node, i) {
                ...
                mesh_path_del(mpath->sdata, mpath->dst,mpath->orig);
        }
        rcu_read_unlock();
}
...
void print_route(void)
{
	...
	u8 *next_hop;
	rcu_read_lock();
	printk("src:	dst:	next_hop(major or minor):	hopcount:\n");
	tbl = rcu_dereference(mesh_paths);
	...
	for_each_mesh_entry(tbl, node, i) {
		...
		printk("%pM\n",((rcu_dereference(mpath->next_hop))->sta).addr);
		

		//printk("%pM\t%pM\t%pM(%1d)\t%d\n",mpath->orig,mpath->dst,
		//				(mpath->next_hop->sta).addr,mpath_is_major,mpath->hop_count);
       // }
		spin_unlock_bh(&mpath->state_lock);
