...
{
	u8 originater_address[ETH_ALEN];
    u32 metric_major;
 	u32 metric_minor;
	u8 * path_list_major;
	u8 major_node_cnt;
...
#ifdef mesh_route
#define PREQ_IE_MID_ADDRESS(X)  (AE_F_SET(x) ? x + 33 : x + 27)
			       ...
			       const u8 *hwmp_ie, enum mpath_frame_type action)  //只为prep调用
{
	...
	case MPATH_PREP:
		...
		mid_address_list=PREP_IE_MID_LIST(hwmp_ie);//
		target_addr=PREP_IE_ORIG_ADDR(hwmp_ie);
	    flags=PERQ_IE_ORIG_FLAGS(hwmp_ie);
	    target_sn=PREQ_IE_ORIG_SN(hwmp_ie);
		break;
	default:
		rcu_read_unlock();
		return 0;
	...
	if (ether_addr_equal(orig_addr, sdata->vif.addr)) {
		...
		if(flags & IEEE80211_PREQ_MAJORPATH) //主路径
		{        
		  mpath = mesh_path_lookup(sdata, orig_addr,target_addr);//建立到目的节点的路径
		  if (mpath) {
          ...
          // 换一个sta 列表最后的一个
          if(ie_len>31+ETH_LEN){
             sta=sta_info_get(sdata,mid_address_list+ie_len-31-ETH_LEN); //?????
             if (!sta) {
		        rcu_read_unlock();
		       return 0;
	         }
         }else{
             sta=sta_info_get(sdata,orig_addr); //?????
             if (!sta) {
		        rcu_read_unlock();
		       return 0;
	         }
		  ...
		  mpath = mesh_path_lookup(sdata,target_addr,orig_addr);//建立到源节点的路径
		  if (mpath) {
			spin_lock_bh(&mpath->state_lock);
			if (mpath->flags & MESH_PATH_FIXED)
				//固定路径 不更新
		...
		else if(flags & IEEE80211_PREQ_MINORPATH)//备份路径
		{
              //同前面一样
		}
				    ...
				    const u8 *preq_elem, u32 metric)
{
	...
	if (ether_addr_equal(target_addr, sdata->vif.addr)) {  // 目的节点 在这里选路？
	...
	//ÊÇµ¥²¥°ü
		...
		char * mid_address_list=kmalloc(hopcount*ETH_ALEN,GFP_ATOMIC);
		if(!mid_address){
			return;
		}
		
	    memcpy(mid_address_list,mid_address,(hop_count-1)*ETH_ALEN);	 
		
	    memcpy(mid_address_list+(hop_count-1)*ETH_ALEN,sdata->vif.addr,ETH_ALEN);		
	...
	if (reply) {
        ...
        destination_node_preq_process(mpath,  orig_addr, ie_len, mid_address_list, metric,lifetime,orig_sn,sdata);
		...
		if (ttl != 0) {
			...
			mesh_path_sel_frame_tx(MPATH_PREP, 0, orig_addr,
					       ...
					       lifetime, metric, 0,mid_address_list, sdata);
		} else {
	...
	if (forward && ifmsh->mshcfg.dot11MeshForwarding) {
		...
		if (flags & IEEE80211_PREQ_PROACTIVE_PREP_FLAG) {   //这里也不会执行
			target_addr = PREQ_IE_TARGET_ADDR(preq_elem);
			target_sn = PREQ_IE_TARGET_SN(preq_elem);
			metric = orig_metric;
		}
								...
								u32 orig_sn, struct ieee80211_sub_if_data *sdata)
{
	...
	if(!node){
		//node=mesh_add_preq_to_list(orig_addr);

		preq_state_machine_add(orig_addr,ie_len,mid_address_list,metric,lifetime,orig_sn,sdata);//至少有主路径
	 ...
	 if (node){
	  ...
	  //&&(!(mpath->flags & MESH_PATH_RECEIVE_PREQ_TIMEOUT)))
	   {  
           ...
           mod_timer(&node->des_req_timer,jiffies+des_req_wait_time);
           mod_timer(&node->des_clear_timer,jiffies+des_req_wait_time+des_req_clear_timeout);
	                                  ...
	                                  struct mesh_preq_state_machine * node,u8 kind)
{
	...
	if(kind==1){
		hash_node=&((node->mesh_preq_mid_list_hash_tabel_major)[hash_num]);
	 	...
	 	hash_node=&((node->mesh_preq_mid_list_hash_tabel_minor)[hash_num]);
    }
...
void mesh_preq_mid_list_hash_tabel_replace(char * mid_address_list_new, u8 count,u8 kind)// kind=1 主 2 备份
  ...
  /* for(i=0;i<count-1;i++){
      ...
      if(hash_node->flags==1){// 添加到链表上
	   ...
	   for(i=0;i<count-1;i++){
	      hash_num=mesh_preq_mid_list_hash(mid_address_list_new+(i*ETH_ALEN),mid_address_list_new+((i+1)*ETH_ALEN));
	      hash_node=&((node->mesh_preq_mid_list_hash_tabel_major)[hash_num]);
	      mesh_preq_mid_list_hash_tabel_add(mid_address_list_new+(i*ETH_ALEN), mid_address_list_new+((i+1)*ETH_ALEN),
		                                  hash_node,kind)
	   }
...
//kind 1 指主路径 2 指 备份路径
{
    ...
    if(node && count>1 )
	{
		...
		{		
			 // hash_num=mesh_preq_mid_list_hash(mid_address_list_new+(i*ETH_ALEN),mid_address_list_new+((i+1)*ETH_ALEN));
             //hash_node=&((node->mesh_preq_mid_list_hash_tabel)[hash_num]);
            if(mesh_preq_mid_list_hash_tabel_has(mid_address_list_new+(i*ETH_ALEN),mid_address_list_new+((i+1)*ETH_ALEN),kind))
            {
               spin_unlock_bh(&(hash_node->lock));
               return false;
            }
	                       ...
	                       u32 orig_sn, struct ieee80211_sub_if_data *sdata)
{
  ...
  if(link_disconnect(orig_addr,ie_len,mid_address_list,1)){
	   ...
	   /*node->metric_minor=node->metric_major;
	   node->minor_node_cnt=node->major_node_cnt;
	   kfree(node->path_list_minor);
	   node->path_list_minor=kmalloc((node->major_node_cnt)*ETH_ALEN,GFP_ATOMIC);
	   memcpy(node->path_list_minor,node->path_list_major,(node->major_node_cnt)*ETH_ALEN);*/
	   // 交换 把主路径hash表指针给备份路径hash表指针
...
struct mesh_preq_state_machine * mesh_get_preq_from_list(u8 * orig_addr)
{
	struct mesh_preq_state_machine * node;
	struct mesh_preq_state_machine * preq_table_bucket;

	u8 hash_num=mesh_preq_state_machine_hash(orig_addr);
    preq_table_bucket=preq_table[hash_num];
	spin_lock(&preq_table_bucket->state_lock);
	
    list_for_each_entry(node,&preq_table_bucket->list,list)
    {
       if(ether_addr_equal(node->originater_address,orig_addr))
       	  return node;
    }

	spin_unlock(&preq_table_bucket->state_lock);
	return NULL;
}

/*struct mesh_preq_state_machine * (struct mesh_preq_state_machine * node)
{
   spin_lock(&preq_table->state_lock);
   list_del(&node->list)
   spin_unlock(&preq_tableq->state_lock);
...
void preq_state_machine_add(u8 * orig_addr,u8 ie_len,char * mid_address_list,u32 metric,u32 lifetime,
                                  u32 orig_snh, struct ieee80211_sub_if_data *sdata) // 在init table中建立一个全局的指针 指向链表的头结点 
										//这里就在链表上添加一个节点  这里还没办法知道路径列表的大小？？ 加锁？
{
	int i=0;
    
    struct  mesh_preq_state_machine *new_node=kzalloc(sizeof(mesh_prep_path_state_machine),GFP_ATOMIC);
    if (!new_node)
		goto err_node_alloc;

	memcpy(new_node->orig_addr,orig_addr,ETH_ALEN);
    ...
    new_node->des_req_timeout_time=jiffies;

    new_node->des_req_clear_timer.data=(unsigned long)new_node;
	new_node->des_req_clear_timer.function=preq_state_machine_clear_timer;
	new_node->des_req_clear_timer.expires=jiffies;//define
    init_timer(&new_node->des_req_clear_timer);

    new_node->major_node_cnt=(ie_len-37)/ETH_ALEN;
    new_node->metric_major=metric;
    // 下面几个kmalloc需要加上判断不成功的状态 这里仅仅为了方便省略是不好的
    new_node->path_list_major=kmalloc((new_node->major_node_cnt)*ETH_ALEN,GFP_ATOMIC);


    new_node->path_list_minor=kmalloc((3)*ETH_ALEN,GFP_ATOMIC);//先分配一段地址？
...
void preq_state_machine_timer(unsigned long data)   //timer  锁 
{
    struct mesh_preq_state_machine * node=(void *)data;
    spin_lock_bh(&node->lock);
    node->des_req_timer_timeout_time=jiffies;
    node->flags |= MESH_RECEIVE_PREQ_TIMEOUT;
	node->flags |= IEEE80211_PREQ_MAJORPATH;

	u8 ttl = ifmsh->mshcfg.element_ttl;    //这里只回了一个 如果没有问题 再写第二个
	if (ttl != 0) {
			mhwmp_dbg(node->sdata, "replying to the PREQ\n");
			mesh_path_sel_frame_tx(MPATH_PREP, 0, node->originater_address,
					       node->orig_sn_major, 0, node->sdata->vif.addr,
					       ifmsh->sn, , 0, ttl,
					       node->lifetime_major, node->metric_major, 0, node->path_list_major
					       ,(node->major_node_cnt)*ETH_ALEN +37, node->sdata_major);
	} else {
			ifmsh->mshstats.dropped_frames_ttl++;
	}
...
void preq_state_machine_clear_timer(unsigned long data)   //timer  锁 并发 很重要 
{
    struct mesh_preq_state_machine * node=(void *)data;
    spin_lock_bh(&node->lock);

    node->flags &= ~MESH_RECEIVE_PREQ_TIMEOUT;
    node->flags &= ~MESH_NOT_RECEIVE_FIRST_PREQ;
				    ...
				    const u8 *prep_elem, u32 metric) // 发送之前要删除地址列表中的最后一个地址 lengh变
{
	...
	ie_len=PREP_IE_LENGTH(hwmp_ie)-ETH_ALEN; //这里只能改变length
	mid_address_list=PREP_IE_MID_LIST(hwmp_ie);//
				    ...
				    const u8 *perr_elem)
{
	...
	if (mpath) {
		...
		if (mpath->flags & MESH_PATH_ACTIVE &&
			...
			mpath->flags &= ~MESH_PATH_ACTIVE;
			//²»É¾³ý Ö»ÊÇÉèÖÃÎª·Çactive Ö»ÊÇÔÚexpireÖÐÉ¾³ýÃ»ÓÐÊ¹ÓÃµÄÂ·¾¶
			mpath->sn = target_sn;
			spin_unlock_bh(&mpath->state_lock);
			if (!ifmsh->mshcfg.dot11MeshForwarding)
				goto endperr;
