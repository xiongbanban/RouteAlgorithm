...
static struct mesh_preq_state_macchine_table __rcu *preq_table;//目的节点接受preq的头结点

#define des_req_resetFlags_timeout 50 HZ
...
{
	struct mesh_path * path_find=mpath_lookup(rcu_dereference(mesh_paths), dst, sdata,orig);
	if(path_find)
	    return path_find;
	else
...
int mesh_pathtbl_init(void)//新建了一个备份路由表 同时新建了目的节点处理preq时的list
{
    ...
    ls1=kzalloc(sizeof(struct mesh_preq_state_macchine_table),GFP_ATOMIC);
    if (!ls1)
		return -ENOMEM;
	spin_lock_init(&ls1->state_lock);
    INIT_LIST_HEAD(&ls1->list);
